...
def load_data():#(nrows):
    ...
    return data#,dataxl #data,dataxl
...
with st.sidebar:
    ...
    print(idc)
    print(df2xl.loc[df2xl["SK_ID_CURR"]==idc].shape)
    print(df2xl.columns)
    ...
    print('prediction=',resultat)
    st.subheader('Index de risque - Prediction')
    st.write(resultat)
    
    Age=np.round(df2xl.loc[df2xl["SK_ID_CURR"]==idc,'DAYS_BIRTH'].values[0]/-365,decimals=0)
    st.write("Age:",Age,' ans')
    
    Sexe=np.round(df2xl.loc[df2xl["SK_ID_CURR"]==idc,'CODE_GENDER'].values[0]/-365,decimals=0)
    if Sexe==1:
        Sexe="Homme"
    if Sexe==0:
        Sexe="Femme"
    ...
    Feature1list=df0xl.columns
    Feature1 = st.selectbox(
        'Facteur 1:',
        Feature1list)

    Feature2list=df0xl.columns[1:]
    Feature2 = st.selectbox(
        'Facteur 2:',
        Feature2list)
    
# si on tick on filtre les données du meme cluster que le client
print(df2xl.loc[df2xl["SK_ID_CURR"]==idc,"cluster"].values[0])
if st.checkbox('Filtre par groupes clients'):
    clu=df2xl.loc[df2xl["SK_ID_CURR"]==idc,"cluster"].values[0]
...
#     st.write(df3xl.loc[df3xl["SK_ID_CURR"]==idc])

# col1, col2, col3 = st.columns(3)
# with col1:
#     x0=df2xl.loc[df2xl.PREDproba>0.5,"PREDproba"]
#     x1=df2xl.loc[df2xl.PREDproba<0.5,"PREDproba"] # ici on a beaucoup de cas avec proba>0.5 du fait de la selection par classe 
#     fig1 = go.Figure()
#     fig1.add_trace(go.Histogram(x=x0,name="lo risk"))
#     fig1.add_trace(go.Histogram(x=x1,name="hi risk"))
#     print(resultat)
#     fig1.add_vline(x=resultat, line_dash = 'dash', line_color = 'firebrick')
...
# with col2: 
#     #x0=df3[Feature1] 
#     x0=df3xl.loc[df2xl.PREDproba>0.5,Feature1]
#     x1=df3xl.loc[df2xl.PREDproba<0.5,Feature1] # inhomogene a cause de l'absence de selection sur ce facteur.
 
#     x_idc2=df3xl.loc[df3xl["SK_ID_CURR"]==idc][Feature1] 
#     print(x_idc2.values)
#     fig2 = go.Figure()
...
# with col3:
#     x0=df3xl.loc[df3xl.PREDproba>0.5,Feature2]
#     x1=df3xl.loc[df3xl.PREDproba<0.5,Feature2]
#     x_idc3=df3xl.loc[df3xl["SK_ID_CURR"]==idc][Feature2]     
#     fig3 = go.Figure()
...
#     go.Scatter(
#         mode='markers',
#         x=df3xl.loc[df3xl["SK_ID_CURR"]==idc][Feature1],
#         y=df3xl.loc[df3xl["SK_ID_CURR"]==idc][Feature2],        
#         name="Client risk",
#         marker=dict(
#             color='black',
#             size=30,
